...
# sudoku = Tk()
# sudoku.title("Sudoku")
# sudoku.iconbitmap('su.jfif')
# sudoku.geometry('650x300')
# l=Label(sudoku,text="Sudoku",font=" Helvetica 20 bold ", fg="darkgrey",bg="green").pack()
# lst=Listbox(sudoku ,width=90,height=90).pack()
# def func():
#     sudoku.destroy()
# btn = Button(sudoku ,text="close window",command=func, fg="red",font="helvetica 10 bold").place(x=0,y=0)
#
# # menu
# # my_menu = Menu(sudoku)
# # sudoku.config(menu=my_menu)
# #
...
def solve_sudoku():
    global number_list
    if mani.check_correct(number_list) != 0:
        mani.solve(number_list)
    else:
        tkinter.messagebox.showerror('invalid',"Invalid")
    display_numbers()

button_solve = tk.Button(win, text = 'slove', command = solve_sudoku)
button_clear_all = tk.Button(win, text = 'clear all', command = clear_list_all)
button_clear = tk.Button(win, text = 'delete element', command = clear_list_sp)
...
menu_file.add_cascade(label = 'Edit', menu = edit_bar)
